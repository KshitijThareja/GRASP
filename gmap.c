...
static void worker_for(void *_data, long i, int tid) // kt_for() callback
{
	...
	if (s->p->opt->flag & MG_M_PARTITION) {
		// Dispatch Read
		int partition_id = graphOp->dispatch_read(qseqs[0], qlens[0], 15, 10);
		graphOp->write_read_to_partition(partition_id, s->seq[off].name, qseqs[0], s->seq[off].qual);
		// if (mg_verbose >= 3)
		// 	fprintf(stderr, "[Dispatcher] Read %s routed to Partition %d\n", s->seq[off].name, partition_id);
	} else {
		// Normal Alignment
		// fprintf(stderr, "[DEBUG] worker_for: processing read %s (len=%d)\n", s->seq[off].name, qlens[0]);
		if (s->p->opt->flag & MG_M_INDEPEND_SEG) {
			for (j = 0; j < s->n_seg[i]; ++j)
				mg_map_frag(s->p->gi, 1, &qlens[j], &qseqs[j], &s->gcs[off+j], b, s->p->opt, s->seq[off+j].name);
		} else {
			// fprintf(stderr, "[DEBUG] Calling mg_map_frag...\n");
			mg_map_frag(s->p->gi, s->n_seg[i], qlens, qseqs, &s->gcs[off], b, s->p->opt, s->seq[off].name);
			// fprintf(stderr, "[DEBUG] mg_map_frag returned.\n");
		}
	}
...
static void *worker_pipeline(void *shared, int step, void *in)
{
	...
	fprintf(stderr, "[DEBUG] worker_pipeline: step %d\n", step);
...
int mg_map_file_frag(const mg_idx_t *idx, int n_segs, const char **fn, const mg_mapopt_t *opt, int n_threads, double *c_seg, double *c_link)
{
	int i, pl_threads;
	pipeline_t pl;
	fprintf(stderr, "[DEBUG] mg_map_file_frag: entry. n_segs=%d\n", n_segs);
	if (n_segs < 1) return -1;
	memset(&pl, 0, sizeof(pipeline_t));
	pl.n_fp = n_segs;
	pl.fp = open_bseqs(pl.n_fp, fn);
	if (pl.fp == 0) {
		fprintf(stderr, "[DEBUG] mg_map_file_frag: open_bseqs failed.\n");
		return -1;
	}
	fprintf(stderr, "[DEBUG] mg_map_file_frag: open_bseqs success.\n");
	pl.opt = opt, pl.gi = idx;
	pl.n_threads = n_threads > 1? n_threads : 1;
	pl.mini_batch_size = opt->mini_batch_size;
	pl.c_seg = c_seg, pl.c_link = c_link;
	pl_threads = n_threads == 1? 1 : (opt->flag&MG_M_2_IO_THREADS)? 3 : 2;
	fprintf(stderr, "[DEBUG] mg_map_file_frag: calling kt_pipeline with %d threads...\n", pl_threads);
	kt_pipeline(pl_threads, worker_pipeline, &pl, 3);
	fprintf(stderr, "[DEBUG] mg_map_file_frag: kt_pipeline returned.\n");
...
int mg_map_files(gfa_t *g, int n_fn, const char **fn, const mg_idxopt_t *ipt, const mg_mapopt_t *opt0, int n_threads)
{
	...
	fprintf(stderr, "[DEBUG] mg_map_files: calling mg_index...\n");
	if ((gi = mg_index(g, ipt, n_threads, &opt)) == 0) return -1;
	fprintf(stderr, "[DEBUG] mg_map_files: mg_index returned. gi=%p\n", gi);
	...
	if (opt.flag & MG_M_FRAG_MODE) {
		...
		for (i = 0; i < n_fn; ++i) {
			fprintf(stderr, "[DEBUG] mg_map_files: calling mg_map_file_frag for file %d...\n", i);
			ret = mg_map_file_frag(gi, 1, &fn[i], &opt, n_threads, cov_seg, cov_link);
			fprintf(stderr, "[DEBUG] mg_map_files: mg_map_file_frag returned %d\n", ret);
			if (ret != 0) break;
		}
	}
	if (opt.flag & MG_M_CAL_COV) {
