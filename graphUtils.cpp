...
void SCC(std::vector<std::vector<int> > &adj, int n_vtx)
{
    ...
    std::vector<std::vector<int> > component_idx(num_cid); // Keep component_idx
    // Map Components
    ...
    {   
        ...
        {   
            ...
            {   
                int neighbor = adj_[j][k];
                if(visited[neighbor]==true)
                {
                    adj_scc[cid][local_map[j]].push_back(local_map[neighbor]);
                }
            }
        }
        for (size_t i=0; i<conn_comp[cid].size(); i++)
    ...
    {   
        ...
        for (size_t local_j = 0; local_j < component_idx[cid].size(); ++local_j) {
             int global_j = component_idx[cid][local_j];
             for(auto &u : adj_scc[cid][local_j]) {
                 adj[global_j].push_back(component_idx[cid][u]);
             }
        }
...
void graphUtils::read_graph()
{
    ...
    for (v = 0; v < n_vtx; v++)
    {
        ...
        {
            // int v_ = av->v_lv >> 32; // Unsafe if n_edges == 0
            // std::cerr << " node : " << v << " node_len : " << node_len[v] << std::endl; 
            for (int i = 0; i < n_edges; i++)
            {
                uint32_t w = av[i].w;
...
void graphUtils::Connected_components()
{
    ...
    for (size_t cid = 0; cid < num_cid; cid++)
    {
        ...
        // Use a local map for global->local index conversion to save memory
        std::unordered_map<int, int> local_map;
        for (size_t j = 0; j < conn_comp[cid].size(); j++) {
            local_map[conn_comp[cid][j]] = j;
        }
        ...
        {   
            ...
            {   
                int neighbor = adj_[j][k];
                if (local_map.find(neighbor) != local_map.end()) {
                    adj_cc[cid][local_map[j]].push_back(local_map[neighbor]); 
                }
            }
        }
    }
...
bool compareAnchors(const Anchors &a, const Anchors &b) {
...
{   //std::chrono::time_point<std::chrono::system_clock> start, end;
    ...
    {
        uint32_t v = anchors[i].x >> 32;
        int cid = component[v];
        Anchors M_; // Anchor
        int minimizer_len = (int32_t)(anchors[i].y>>32&0xff);
        int node = (int)(anchors[i].x>>32);
        M_.v = node;
        M_.c = (int32_t)(anchors[i].y) - minimizer_len + 1;
        M_.d =  (int32_t)(anchors[i].y);
        M_.x =  (int32_t)(anchors[i].x) - minimizer_len + 1;
        M_.y =  (int32_t)(anchors[i].x);
        M[cid].push_back(M_);
        idx_Anchor[cid].push_back(anchors[i]);
    }
    ...
    for (size_t cid = 0; cid < num_cid; cid++)
    {
        if (M[cid].size() == 0) continue;
        
        // Sorting
        std::sort(M[cid].begin(), M[cid].end(), compareAnchors);
...
{
    // fprintf(stderr, "Dispatching read len=%d\n", len);
    ...
    if (votes.empty()) return 0; // Default to partition 0
    ...
    // fprintf(stderr, "Dispatched to %d with %d votes\n", best_pid, max_votes);
    return best_pid;
}

void graphUtils::write_read_to_partition(int pid, const char *name, const char *seq, const char *qual)
{
    if (pid < 0 || pid >= partition_fps.size()) {
        fprintf(stderr, "Error: pid %d out of bounds (size %zu)\n", pid, partition_fps.size());
        return;
    }
    FILE *fp = partition_fps[pid];
    if (!fp) {
        fprintf(stderr, "Error: partition_fps[%d] is null\n", pid);
        return;
    }
