...
class graphUtils
{
    ...
    void build_global_index(int k, int w);
    int dispatch_read(const char *seq, int len, int k, int w);
    void open_partition_read_files(const char *prefix);
    void close_partition_read_files();
    void write_read_to_partition(int pid, const char *name, const char *seq, const char *qual);
