...
void mg_map_frag(const mg_idx_t *gi, int n_segs, const int *qlens, const char **seqs, mg_gchains_t **gcs, mg_tbuf_t *b, const mg_mapopt_t *opt, const char *qname)
{
	...
	*/
	...
	for (size_t i = 0; i < n_a; i++)
	{
	    ...
	    // Chaining
    // fprintf(stderr, "[DEBUG] Before Chaining. n_a=%zu\n", anchors.size());
    std::vector<mg128_t> best; // Declare best here
    if (graphOp != NULL) {
        best = graphOp->Chaining(anchor); // Use 'anchor' not 'anchors'
    } else {
        // Fallback or error
        // fprintf(stderr, "[ERROR] graphOp is NULL in mg_map_frag\n");
    }
    // fprintf(stderr, "[DEBUG] After Chaining. best.size()=%zu\n", best.size());
	kfree(b->km, a);
